// get_holders_ethplorer.js (ФИНАЛЬНАЯ ВЕРСИЯ 5.0: ГИБКИЙ ПОИСК ПО ВРЕМЕНИ)
...
const CONFIG = {
    ...
    apiPauseMs: 2000,
    // НОВАЯ НАСТРОЙКА: "Окно" для поиска ближайшей записи в минутах (±10 минут)
...
(async () => {
    // ... (весь код до `historyQuery` остается без изменений) ...
    ...
    for (const record of newRecords) {
        ...
        const historyQuery = `
            WITH vars AS (
                SELECT 
                    NOW() - INTERVAL '1 hour' AS h1_target,
                    NOW() - INTERVAL '3 hours' AS h3_target,
                    NOW() - INTERVAL '12 hours' AS h12_target,
                    NOW() - INTERVAL '24 hours' AS h24_target,
                    INTERVAL '${CONFIG.searchWindowMinutes} minutes' AS window
            )
            SELECT
                -- Для предыдущей записи логика остается той же - берем вторую с конца
                (SELECT h.holders FROM holders h WHERE h.contract ILIKE $1 ORDER BY h.parsed_at DESC LIMIT 1 OFFSET 1) AS prev_holders,
                
                -- Логика для 1 часа
                (SELECT h.holders FROM holders h, vars
                 WHERE h.contract ILIKE $1 AND h.parsed_at BETWEEN (vars.h1_target - vars.window) AND (vars.h1_target + vars.window)
                 ORDER BY ABS(EXTRACT(EPOCH FROM (h.parsed_at - vars.h1_target)))
                 LIMIT 1) AS h1_holders,

                -- Логика для 3 часов
                (SELECT h.holders FROM holders h, vars
                 WHERE h.contract ILIKE $1 AND h.parsed_at BETWEEN (vars.h3_target - vars.window) AND (vars.h3_target + vars.window)
                 ORDER BY ABS(EXTRACT(EPOCH FROM (h.parsed_at - vars.h3_target)))
                 LIMIT 1) AS h3_holders,

                -- Логика для 12 часов
                (SELECT h.holders FROM holders h, vars
                 WHERE h.contract ILIKE $1 AND h.parsed_at BETWEEN (vars.h12_target - vars.window) AND (vars.h12_target + vars.window)
                 ORDER BY ABS(EXTRACT(EPOCH FROM (h.parsed_at - vars.h12_target)))
                 LIMIT 1) AS h12_holders,

                -- Логика для 24 часов
                (SELECT h.holders FROM holders h, vars
                 WHERE h.contract ILIKE $1 AND h.parsed_at BETWEEN (vars.h24_target - vars.window) AND (vars.h24_target + vars.window)
                 ORDER BY ABS(EXTRACT(EPOCH FROM (h.parsed_at - vars.h24_target)))
                 LIMIT 1) AS h24_holders
        `;
